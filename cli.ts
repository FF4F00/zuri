...
async function Main(appName: string, flag: string) {
  try {
    if (command[cmd]) {
      ...
      console.log(`zuri 1.0`);
    }
    else {
      console.log(`To create a project, type:` + ` %czuri create ` + `%c[project name]`, "color:#55dac8;", "color:red;");
      console.log(`To compile a project, type:` + ` %czuri build`, "color:#55dac8;");
      console.log(`To start a dev server, type:` + ` %czuri dev`, "color:#55dac8;");
    }
  } catch (error) {
