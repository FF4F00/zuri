...
async function Main(appName: string, flag: string) {
  try {
    if (command[cmd]) {
      ...
      console.log(`To create a project, type:` + ` %cvia create ` + `%c[project name]`, "color:#55dac8;", "color:red;");
      console.log(`To compile a project, type:` + ` %cvia build`, "color:#55dac8;");
      console.log(`To start a dev server, type:` + ` %cvia dev`, "color:#55dac8;");
    }
  } catch (error) {
